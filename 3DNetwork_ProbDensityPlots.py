...
class Adj_Mats(object):
    ...
    def make_eigenvalues(self, hamiltonian_iter=5):
...
def whichBin(binnumber,array):
    print(array)
    instances=np.zeros((len(array),binnumber))
    probability=np.zeros((len(array),binnumber))
...
types='all_frames'
lowerlimit=1
upperlimit_list=createUpperLimitList(lowerlimit,5,1)
spacinghistograms=[]
framesindices=np.linspace(0,2500,3)
...
for upperlimit in upperlimit_list:
    ...
    if __name__ == "__main__":
        ...
        if (types=='all'):
            ...
            #hist, xedges, yedges = ax.hist2d(allhists, x_coord, bins=numberofbins, weights=weighting)
...
allspacings_list=np.array(spacinghistograms)
...
if (types=='all'):
    ...
    #spacinginfo[2]
    eigenvaluearray=np.array(valueiterations)

    #XY-plane
    yy=np.linspace(valuemin,valuemax,valueiterations)
    ...
    allspacings_list=np.array(allspacings_list)
    Z = allspacings_list
    print(X.shape)
    print(Y.shape)
    print(Z.shape)
    ...
    xx=np.linspace(framemin,framemax,frameiterations)
    ...
    for frame in range(len(framesindices)):
        for i in range(len(allspacings_list)):
            ...
            weighting = np.ones_like(allspacings_list[frame][i]) / len(allspacings_list[frame][i])
...
#7) (Spacing=Y) vs (ordered eigenvalue pairs=X) vs (probability density of spacing=Z)


##Extra Code:
'''
for col in range(len(allspacings_list)):
if col==0:  
    for frame in range(len(allspacings_list[col])):
        axes.hist(allspacings_list[col][frame], bins = spacingbinvalues, alpha=0.5, weights=weighting)
        yy = upperlimit_list
        X, Y = np.meshgrid(xx,yy)
        ax = fig.add_subplot(1, 2, 1, projection='3d')
        surf = ax.plot_surface(X, Y, Z, rstride=1, cstride=1, cmap=cm.coolwarm, linewidth=0, antialiased=False)
        fig.colorbar(surf, shrink=0.5, aspect=10)
        ax.set_xlabel(frameindices)
...
'''
xpos, ypos = np.meshgrid(xedges[:-1], yedges[:-1]+0.125, indexing="ij")
xpos = xpos.ravel()
ypos = ypos.ravel()
zpos = 0

# Construct arrays with the dimensions for the 16 bars.
spacingaxis_scaling=(max(allhists)-min(allhists))/numberofbins
dx = np.ones_like(zpos)*spacingaxis_scaling
#*(1/numberofbins)
dy = np.ones_like(zpos)*0.75
dz = hist.ravel()
...
ax.bar3d(xpos, ypos, zpos, dx, dy, dz, zsort='average', alpha=0.6, cmap=plt.cm.jet)
