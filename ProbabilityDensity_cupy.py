...
for upperlimit in upperlimit_list:
    ...
    if __name__ == "__main__":
        ...
        if (types=='all'):
            ...
            x = np.array(createUpperLimitList(0, 2500, len(spacinginfo)))
            x_coord = np.repeat(x, len(spacinginfo[0]))
            spacing_array = cp.asnumpy(spacinginfo)
            ...
            ax.hist2d(allhists, x_coord, bins=numberofbins, weights=weighting, range=[[0, 0.9], [0, 2500]])
