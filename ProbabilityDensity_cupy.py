...
class Adj_Mats(object):
    ...
    def get_atom_dists(self):
        ...
        dists = cp.reshape(base, (len(framesindices), 1, len(frames[0]), 3)) - np.reshape(base, (len(framesindices), len(frames[0]), 1, 3))  
        dists = dists**2
        dists = dists.sum(3)
        dists = cp.sqrt(dists)
...
types='median'
lowerlimit=1
#upperlimit_list=createUpperLimitList(lowerlimit,2.2,0.05)
upperlimit_list=[1,10]
spacinghistograms=[]
framesindices=[0,999,2499]
