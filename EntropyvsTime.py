...
class Adj_Mats(object):
    ...
    def get_atom_dists(self):
        ...
        for i in range(len(frameindex)):
            baseindex=i
            current_frame=int(frameindex[i])
            for j in range(len(frames[current_frame])):
                for k in range(len(frames[current_frame][j])):
                    base[baseindex][j][k] = frames[current_frame][j][k]
